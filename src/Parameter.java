/**
 * This class stores parameters for the model, like number of people, people’s max vision and so on.
 */
public class Parameter {

    //World information
    public static int MAX_GRAIN = 50;
    public static int MAX_WORLD_X = 50;
    public static int MAX_WORLD_Y = 50;
    public static int MAX_TIME_TICK = 900;

    //variables
    public static int NUM_PEOPLE = 250;
    public static int MAX_VISION = 5;
    public static int METABOLISM_MAX = 15;
    public static int LIFE_EXPECTANCY_MIN = 1;
    public static int LIFE_EXPECTANCY_MAX = 100;
    public static int PERCENT_BEST_LAND = 10;
    public static int GRAIN_GROWTH_INTERVAL = 1;
    public static int NUM_GRAIN_GROWN = 4;
    public static double SPREAD_PERCENTAGE = 0.25;
    public static double INHERITED_PERCENTAGE = 0;

}
